...
/// ICMPv6 Destination Unreachable message (Type 1).
///
/// ```text
/// 0                   1                   2                   3
...
pub struct Icmpv6DestUnreach<T>
where
    T: AsRef<[u8]>,
{
    data: T,
...
impl<T: AsRef<[u8]>> AsRef<[u8]> for Icmpv6DestUnreach<T> {
    fn as_ref(&self) -> &[u8] {
        self.data.as_ref()
    }
}

impl<T: AsRef<[u8]> + AsMut<[u8]>> AsMut<[u8]> for Icmpv6DestUnreach<T> {
...
mod tests {
    ...
    #[test]
    fn test_dest_unreach_parse() {
        let data = vec![
            0x01, 0x04, // type=1, code=4 (port unreachable)
            0x00, 0x00, // checksum
        ...
        let msg = Icmpv6DestUnreach::new(&data[..]).unwrap();
